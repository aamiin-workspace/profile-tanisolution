...
export default function KolaborasiManager() {
  ...
  const resetForm = () => {
    setFormData({ 
        title: '', category: 'magang', caption: '', 
        detail: '', extra_1: '', extra_2: '', date: '', image: null 
    });
    setPreview(null);
    setIsEditing(false);
    setEditId(null);
  ...
  const handleDeleteProcess = async (id) => {
    try {
        ...
        if (res.ok) {
            fetchCollabs();
            setTimeout(() => {
                showModal('success', 'Berhasil Dihapus', 'Data kolaborasi berhasil dihapus.');
            }, 300);
        } else {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        fetchCollabs(); 
        resetForm(); 
        showModal('success', 'Berhasil Disimpan', isEditing ? 'Data kolaborasi berhasil diperbarui!' : 'Data baru berhasil ditambahkan!');
      } else {
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md border border-gray-200 dark:border-gray-700 min-h-screen">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        {view === 'list' && (
          <button onClick={handleOpenCreate} className="bg-primary text-white px-4 py-2 rounded-lg hover:bg-green-700 transition shadow">
            + Tambah Baru
          </button>
        )}
      </div>
                ...
                <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                  ...
                  <td className="px-4 py-3">
                      <span className={`px-2 py-1 rounded-full text-xs font-bold capitalize
                        ${item.category === 'magang' ? 'bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-300' : 
                          item.category === 'riset' ? 'bg-blue-100 text-blue-800 dark:bg-blue-900/30 dark:text-blue-300' : 
                  ...
                  <td className="px-4 py-3 text-xs italic text-gray-500 dark:text-gray-400">
                      {item.category === 'riset' ? `Peneliti: ${item.extra_1 || '-'}` : 
                       item.category === 'kunjungan' ? `Instansi: ${item.extra_1 || '-'}` : '-'}
                  </td>
      ...
      {view === 'form' && (
        <form onSubmit={handleSubmit} className="space-y-4 max-w-3xl mx-auto bg-gray-50 dark:bg-gray-700/50 p-6 rounded-lg border border-gray-200 dark:border-gray-600">
          ...
          <div>
            <label className={labelClass}>Upload Foto Dokumentasi {isEditing && "(Opsional)"}</label>
            <input 
                type="file" 
                id="collabFileInput"
            ...
            {preview && (
                <div className="mt-4 relative w-full h-32">
                    <img src={preview} alt="Preview" className="h-full rounded shadow object-cover" />
                </div>
            )}
          </div>

          <div>
            <label className={labelClass}>Caption Singkat (Muncul di Galeri)</label>
             ...
             <div className="grid grid-cols-1 gap-4">
                ...
                {(formData.category === 'riset' || formData.category === 'kunjungan') && (
                    <div>
                        ...
                        <input type="text" name="extra_1" value={formData.extra_1} onChange={handleChange} className={inputClass} placeholder="Kosongkan jika tidak perlu" />
                    </div>
                )}
                ...
                {formData.category === 'riset' && (
                    <div>
                        ...
                        <textarea name="extra_2" value={formData.extra_2} onChange={handleChange} className={inputClass} rows="3" placeholder="Ringkasan hasil riset..."></textarea>
                    </div>
                )}

                {formData.category === 'magang' && (
                    <p className="text-sm text-gray-500 italic">Tidak ada detail khusus untuk kategori ini.</p>
                )}
             </div>
          </div>

          <div>
            <label className={labelClass}>Uraian Detail (Muncul di Popup)</label>
            <textarea name="detail" rows="5" value={formData.detail} onChange={handleChange} className={inputClass} required></textarea>
          </div>
