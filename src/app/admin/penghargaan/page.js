...
export default function AchievementManager() {
  ...
  const resetForm = () => {
    setFormData({ category: 'award', title: '', year: '', description: '', image: null });
    setPreview(null); 
    setIsEditing(false); 
    setEditId(null); 
    setView('list');
  ...
  return (
    <div className="bg-white dark:bg-gray-800 p-6 rounded-xl shadow-md border border-gray-200 dark:border-gray-700 min-h-screen">
      ...
      <div className="flex justify-between items-center mb-6">
        ...
        {view === 'list' && (
            <button onClick={() => { resetForm(); setView('form'); }} className="bg-primary text-white px-4 py-2 rounded-lg hover:bg-green-700 shadow transition">+ Tambah Data</button>
        )}
      </div>
      ...
      {view === 'list' ? (
                    ...
                    {loading ? (
                        ...
                        <tr key={item.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50 transition">
                            ...
                            <td className="p-3 text-center space-x-2">
                                ...
                                <button onClick={() => confirmDelete(item.id)} className="text-red-600 hover:text-red-800 font-bold"><i className="fas fa-trash"></i> Hapus</button>
                            </td>
                        </tr>
                    ))}
                </tbody>
        ...
        <form onSubmit={handleSubmit} className="space-y-4 max-w-xl mx-auto bg-gray-50 dark:bg-gray-700/50 p-6 rounded-lg border border-gray-200 dark:border-gray-600">
            ...
            <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                    <label className="block text-sm font-bold mb-1 dark:text-gray-300">Upload Foto</label>
            ...
            <div>
                <label className="block text-sm font-bold mb-1 dark:text-gray-300">Deskripsi Singkat</label>
